...
class PeleeNet(nn.Module):
    ...
    def __init__(self):#phase, size, cfg=None):
        ...
        #self.phase = phase #模式：train or test
        #self.size = size
        #self.cfg = cfg
        print("in peleenet init!")
        ...
        for i, x in enumerate([256] * 5):
            n = self.anchor_config['anchor_nums'][i]
            self.loc.append(nn.Conv2d(x, n * 4, kernel_size=1))
            self.conf.append(nn.Conv2d(x, n * self.num_classes, kernel_size=1))
        
        #######################################加###################################
        self.bn1 = nn.BatchNorm2d(704) 
        self.linear0 = nn.Linear(704, 64)
        self.linear1 = nn.Linear(64, 15) 
        #######################################加###################################

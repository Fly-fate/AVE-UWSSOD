...
def double_lrelu(x, leak=0.1, name="double_lrelu"): #没用
  with tf.variable_scope(name):
    return tf.minimum(tf.maximum(leak * x, x), leak * x - (leak - 1))


# clamp to lower, upper; leak is RELATIVE
def leaky_clamp(x, lower, upper, leak=0.1, name="leaky_clamp"): #没用
  with tf.variable_scope(name):
...
def tanh_range(l, r, initial=None): #用
...
def load_config(config_name): #没用
  scope = {}
...
# added by Hao He
# ======================================================================================================================
def get_artist_batch(folder, size=128, num=64): #没用
  import os
...
def degrade_images_in_folder( #没用
    folder,
    dst_folder_suffix,
    LIGHTDOWN=True,
    UNBALANCECOLOR=True,):
...
def vis_images_and_indexs(images, features, dir, name):#没用
...
class UtilImageError(Exception): #没用
  pass
...
def linearize_ProPhotoRGB(pp_rgb, reverse=False):#没用
  if not reverse:
    gamma = 1.8
  else:
    gamma = 1.0 / 1.8
...
def XYZ_chromatic_adapt(xyz, src_white='D65', dest_white='D50'):#没用
...
def ProPhotoRGB2XYZ(pp_rgb, reverse=False):#没用
  if not reverse:
...
def normalize_Lab_image(lab_image):#没用
...
def read_tiff_16bit_img_into_XYZ(tiff_fn, exposure=0):#没用
...
def linearProPhotoRGB2Lab(img):#没用
  if not img.shape[2] == 3:
    print('pp_rgb shape', img.shape)
    raise UtilImageError('image channel number is not 3')
  xyz = ProPhotoRGB2XYZ(img)

...
class DIP:
    def __init__(self, input):
        self.filters = [DefogFilter,ImprovedWhiteBalanceFilter,GammaFilter,ContrastFilter,UsmFilter]
        #self.filters = [DefogFilter,ImprovedWhiteBalanceFilter,GammaFilter,ToneFilter,ContrastFilter,UsmFilter]
        ...
        # self.dark = torch.zeros([self.filter_input.shape[0], self.filter_input.shape[1], self.filter_input.shape[2]])
        # self.defog_A = torch.zeros([self.filter_input.shape[0], self.filter_input.shape[3]])
        # self.IcA = torch.zeros((self.filter_input.shape[0], self.filter_input.shape[1], self.filter_input.shape[2]))
    ...
    #def image_process(self,filtered_image_batch,filter_features,defog_A, IcA):
        ...
        for j, filter in enumerate(filters): #遍历每个filter #构建filter
            ...
            filtered_image_batch, filter_parameter = filter.apply(filtered_image_batch, filter_features, self.defog_A, self.IcA)
